...
public:
	...
	BOOL StartDial();
	BOOL bSpeedChecked;			// надо 1 ра?проверит?скорость
	BOOL bAuthenticationBegin;	// надо 1 ра?запустит?внешни?программ?
	CString sCurrentProvider;
	BOOL bDialing;			// dial button in "pressed" stage
	BOOL bDialNow;			// нель? входит??OnTimer (Запущена RasDial, ждем ответа)
							// инач?таймер запуще? но звонит?не надо
	BOOL bConnected;		// ?онлайн?
	BOOL bInOnTimer;		// ?обработчик?онтаймер
	BOOL bPhone;			// Курсор ?дереве стои?на телефона?
	CTime tBeginDial;		// момент запуск?rasDia, нуже?для timeout
