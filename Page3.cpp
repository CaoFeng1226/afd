...
void CPage3::OnTimer(UINT nIDEvent)
{
	...
	// проыерка на пиновани?сервер?
	if (m_bPing && bConnected)
	{
		CTime ttt = CTime::GetCurrentTime();
		CTimeSpan ts = ttt - LastPingCheck;
		if (ts.GetTotalSeconds() >= atol(m_sPing) * 60)  // время ?секундах
		{
			LastPingCheck = CTime::GetCurrentTime();
			PingISPServer();
		}
	}
	// проверка необходимост?разрыв?сесс?пр?низком траффике
...
{
	SaveAllToIni();
//	пеуcтановить начальны?период для ping'a
	LastPingCheck = CTime::GetCurrentTime();
}
...
{
	SaveAllToIni();
//	пеуcтановить начальны?период для подсчета траффика
	LastTrafficCheck = CTime::GetCurrentTime();
}
